...
void enCQueue(QueueType* cQ, element item) {
	if (isCQueueFull(cQ)) return;
		...
		cQ->rear = (cQ->rear + 1) % cQ_SIZE;	//rear 포인터 이동
...
element deCQueue(QueueType* cQ) {
	if (isCQueueEmpty(cQ)) return;
		...
		cQ->front = (cQ->front + 1) % cQ_SIZE;	//front 포인터 이동
		return cQ->queue[cQ->front];				//item 반환
	}
}
...
void printCQ(QueueType* cQ) {
	int i, first, last;
	first = (cQ->front + 1) % cQ_SIZE;	//첫 번째 원소의 위치
	last = (cQ->rear + 1) % cQ_SIZE;	//마지막 원소의 위치
